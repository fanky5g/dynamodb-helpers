...
import (
	...
	"gitlab.com/fanky5g/paysenger/server/config"
	"gitlab.com/fanky5g/paysenger/server/types"
	"gitlab.com/fanky5g/paysenger/server/util"
	// "time"
	"os"
)
...
func getDynamodbInstance() (*dynamodb.DynamoDB, error) {
	...
	isLocalDev := os.Getenv("PAYSENGER_LOCAL_DEV")

	if isLocalDev != "" && isLocalDev == "true" {
		sess, err := config.GetAWSSession(cfg.AWSDefaultRegion, "http://localhost:8000")
		if err != nil {
			return nil, err
		}
